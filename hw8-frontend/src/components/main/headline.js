...
import { addNewPostWithImage } from './mainactions'

//The headline component to post a new article
const Headline = ({ addNewPostWithImage, author }) => {
    let inputArticle;
    let inputImg;
            ...
            <div className="col-md-6 col-sm-6" className="postButtons">
                <div className="btn-group" role="group" className="postButtons">
                    ...
                    <input id="articlesubmit" className="btn btn-success" type="button" value="Post" onClick={ ()=>{
                        if ((inputArticle && inputArticle.value != '') || inputImg) {
                            addNewPostWithImage(author, inputArticle.value, inputImg)
                            inputArticle.value=''
                            inputImg = undefined
                        }}}/>
                </div>
...
export default connect(
    ...
    (dispatch) => {
        return {
            addNewPostWithImage: (author, text, image) => dispatch(addNewPostWithImage(author, text, image))
        }
    }
)(Headline)
